        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-body">
                    ...
                    <form action="{{route('district.update',$dis->id)}}" method="post">
                        @csrf
                        @method('PATCH')
                        <div class="form-group">
                            <label>District Name</label>
                            <input type="text" class="form-control" name="name" value="{{$dis->name}}"/>
                        @error('name')
                            ...
                            <select name="division_id" class="form-control w-100">
                                @foreach ($division as $data)
                                    <option value="{{ $data->id }}"
                                        @if ($data->id === $dis->division_id) selected @endif>
                                        {{ $data->name }}
                                    </option>
                                @endforeach
                            </select>
